...
__export(rules_exports, {
  ...
  behindBaseRule: () => behindBaseRule,
  changesRequestedRule: () => changesRequestedRule,
  ciBrokenOnBaseRule: () => ciBrokenOnBaseRule,
  ciFailingRule: () => ciFailingRule,
  ciMissingRule: () => ciMissingRule,
  ciPendingRule: () => ciPendingRule,
  ciRules: () => ciRules,
  deriveStatus: () => deriveStatus,
  draftRule: () => draftRule,
  duplicateFilesRule: () => duplicateFilesRule,
  duplicatePrRule: () => duplicatePrRule,
  firstTimeContributorRule: () => firstTimeContributorRule,
  hugeDiffRule: () => hugeDiffRule,
  mergeConflictRule: () => mergeConflictRule,
  newDependencyRule: () => newDependencyRule,
  noDcoRule: () => noDcoRule,
  noTestChangedRule: () => noTestChangedRule,
  possibleSecretRule: () => possibleSecretRule,
  runRules: () => runRules,
  staleRule: () => staleRule,
  testsRemovedRule: () => testsRemovedRule,
  touchesProtectedRule: () => touchesProtectedRule
});
...
var staleRule = {
  code: "STALE",
  run: (context) => {
    const nudgeAfterDays = context.config?.staleNudgeAfterDays ?? context.config?.staleDays ?? DEFAULT_NUDGE_AFTER_DAYS;
...
function renderTerminalReport(prs) {
  const sections = {
    "Ready for you": [],
  ...
  for (const title of sectionKeys) {
    ...
    if (items.length === 0) {
      ...
      for (const item of items) {
        ...
        const ageDays = Math.floor((collectedAt - createdAt) / (1e3 * 60 * 60 * 24));
        let notes = failedCodes.length > 0 ? failedCodes.join(", ") : "";
        if (context.checks === void 0 && context.baseChecks === void 0) {
...
var SECTIONS = [
  {
    ...
    blurb: "A required check is failing here **and was already failing on the base commit**. The contributor did not cause these. Fixing the base clears them all at once."
  },
  {
    key: "readyForYou",
  ...
  },
  {
    ...
    blurb: "Waiting on the author. The bot has already told them what to fix, so these need nothing from you."
  },
  {
    key: "waiting",
    heading: "Waiting",
...
function renderDigest(prs, sectionCap = DEFAULT_SECTION_CAP) {
  ...
  const lines = ["# PR Reviewer digest", ""];
  if (prs.length === 0) {
...
var validateState = ajv.compile(stateSchema);
async function readState(owner, repo, pullNumber, commentBody) {
  ...
  if (commentBody) {
    const match2 = commentBody.match(/<!-- pr-reviewer:v1\s*({.*?})\s*-->/s);
    if (match2 && match2[1]) {
      try {
...
async function writeState(owner, repo, pullNumber, state) {
...
async function applyComment(octokit, owner, repo, pullNumber, results, status, reportMarkdown, dailyEditCap = 10, dryRun = false) {
...
// src/act/digest.ts
var DIGEST_MARKER = "<!-- pr-reviewer:digest:v1 -->";
var DIGEST_TITLE = "PR Reviewer digest";
function digestBody(markdown) {
  return `${DIGEST_MARKER}

${markdown}`;
}
async function applyDigest(octokit, owner, repo, markdown, dryRun = false) {
  ...
  if (dryRun) {
    console.log(
      existing ? `[dry-run] Would update digest issue #${existing.number}.` : "[dry-run] Would create and pin a digest issue."
    );
    return {
      action: "skipped",
...
async function pinIssue(octokit, nodeId) {
  ...
  try {
    ...
    console.warn(
      `Created the digest issue but could not pin it: ${reason}. A repository allows at most three pinned issues, and pinning needs issue-write scope.`
    );
    return false;
  }
}
...
async function recommend(options = {}) {
  ...
  const hasContributing = contributingPaths.some((p) => fs.existsSync(path2.join(dir, p)));
  if (!hasContributing) {
    issues++;
  ...
  const hasPrTemplate = templatePaths.some((p) => fs.existsSync(path2.join(dir, p)));
  if (!hasPrTemplate) {
    issues++;
  ...
  log.info("\u{1F4A1} Recommendation: Enforce Linked Issues");
  log.info(
...
async function collectCheckRuns(octokit, owner, repo, headSha, baseBranch) {
  ...
  try {
    ...
    if (statusChecks) {
      requiredContexts = [
        ...
        ...statusChecks.checks?.map((c) => c.context) || []
      ];
    }
  } catch (error) {
...
async function collectBaseCheckRuns(octokit, owner, repo, baseSha) {
  const checkRuns = await octokit.paginate(octokit.rest.checks.listForRef, {
    owner,
    repo,
    ref: baseSha
...
async function scanCommand(options) {
  ...
  for (const listPr of prs) {
    ...
    const context = {
      ...
      ...checks !== void 0 ? { checks } : {},
      ...baseChecks !== void 0 ? { baseChecks } : {}
    };
...
var ALLOWED_CONFIG_KEYS2 = /* @__PURE__ */ new Set([
  "labelPrefix",
  "disabledRules",
  "dryRun",
  "protectedGlobs",
...
async function loadConfig2(octokit, owner, repo) {
  try {
    const response = await octokit.rest.repos.getContent({
      owner,
      repo,
    ...
    if ("content" in response.data) {
      ...
      for (const key of Object.keys(parsed)) {
        if (!ALLOWED_CONFIG_KEYS2.has(key)) {
          throw new Error(`Unknown config key: '${key}'.`);
        }
      }
      return parsed;
...
async function processPullRequest(octokit, owner, repo, pullNumber, config, dryRun, prefetchedBaseChecks) {
  ...
  const { renderComment } = await import('./render-5JCM6QY6.js');
...
async function sweepOpenPullRequests(octokit, owner, repo, config, dryRun) {
  ...
  const evaluated = [];
  const failures = [];
  const baseChecksMap = /* @__PURE__ */ new Map();
...
async function runCommand() {
  ...
  let config = await loadConfig2(octokit, owner, repo);
  ...
  if (eventName === "pull_request") {
    ...
    console.log(
      `Sweep complete: ${sweep.evaluated.length}/${sweep.total} open PRs evaluated` + (sweep.failures.length > 0 ? `, ${sweep.failures.length} failed` : "")
    );
...
export { act_exports as act, cli_exports as cli, collect_exports as collect, judge_exports as judge, render_exports as render, rules_exports as rules };
